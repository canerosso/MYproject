...
//session_start();
 ...
 public function get_myscore($id){			
			 $myscore=	'SELECT sum( score ) AS score FROM user_result WHERE user_id='.$id;
                
                try{
              		       return parent::excuteQuery($myscore);
			     }
				catch(Exception $e){
				echo $e->getMessage();
...
LIMIT 0 , 30ORDER BY a.created_at ASC ";
	...
	function updateprofile($params){
	       ...
	       $mob=$params['mobile'];
 		   $email=$params['email'];
 		   $query="SELECT * FROM users WHERE (email='$email' OR mobile='$mob') AND id!='$id' ";
 		   $allready= parent::excuteQuery($query);
 		   $exitcount = count($allready);
		...
		$getAllUsers = "SELECT * FROM  `users` where `users`.`user_type`='0' and `users`.`status`=1
        	...
        	function getregid(){
		$getAllUsers = "SELECT `regId` FROM  `users` where `users`.`user_type`='0' and `users`.`status`=1
					 ORDER BY  `users`.`created_at` ASC ";
			try{
                   // $eventsInfo=$parent::excuteQuery($getAllUsers);
				$selectStatement = $this->databaseConnection->prepare($getAllUsers);
