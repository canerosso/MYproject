...
############################### Username Authentication Admin Panel ############    
 ...
 $app->get('/getLogo', function () use ($app) {
    	
		$response = array("filepath"=>URL."logo/logo.png");
     ...
     $params['user_type']=$app->request->post('user_type');
      $params['regId']=$app->request->post('regId');
...
// Register User
 ...
 $app->post('/register_user', function () use ($app) {
		...
		// $where['mobile']=$params['mobile'];
         ...
         $lastInsertId=$userObject->userSignUp($params['name'],$params['mobile'],$params['email'],$params['user_type'],$params['regId']);
         ...
         // echo json_encode($user);
          if (isset($isMobile) && !empty($isMobile)) 
          {
            ...
            {
            	 $user['status']=TAG_RESPONSE_SUCCESS;
                 $user['message']=' You Have Already Registered ';
                 $user['score']='0';    
                 $user['user']=array();
               	 $params['id']=$all[0]->id;
 ...
 $app->post('/myprofile', function () use ($app) {
    $params   =$app->request->post(); // $_POST 
   $api_key  = $params['api_key'];
   $isMobile = $params['isMobile'];
   $id       = $params['id'];
     ...
     if ($co24===1) 
      {
          ...
          $userObject2 = new Users(SERVER_API_KEY);
          $score=$userObject2->user_score($id);
          $myscore =  $score[0]->score;
  ...
  if($api_key==SERVER_API_KEY)
    {
      ...
      $score=$userObject->user_score($id);
       $imgData= $profile[0]->image;
      $myscore =  $score[0]->score;
...
$app->post('/update_user', function () use ($app) {
       ...
       $api_key = @$params['api_key'];
         $isMobile = @$params['isMobile'];
         if($api_key==SERVER_API_KEY  && !empty($isMobile))
         {
            $userObject = new Users(SERVER_API_KEY);
...
$app->post('/get_topper', function () use ($app) 
{
     $params=$app->request->post(); 
       $api_key = @$params['api_key'];
         $isMobile = @$params['isMobile'];
         ...
         {
            $userObject = new Users(SERVER_API_KEY);
            $top = $userObject->get_topper();
              $user['status']=TAG_RESPONSE_SUCCESS;
           $user['message']='Top Five';
           $user['user']=$top; 
